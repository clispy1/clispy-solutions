...
export const metadata: Metadata = {
  ...
  keywords: ["portfolio", "projects", "web design", "web development", "Ghana", "case studies"],
  alternates: {
    canonical: '/projects',
  }
};
      ...
      <main className="container mx-auto px-4 py-32">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-white/80 max-w-2xl mx-auto">
            Explore our portfolio of websites and digital solutions we've built for clients across Ghana and beyond.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            ...
            >
              ...
              <div className="p-6">
                <h2 className="text-xl font-bold mb-2" itemProp="name">{project.title}</h2>
                <p className="text-white/60 mb-4" itemProp="description">{project.description}</p>
                
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.technologies.map((tech) => (
                    <span key={tech} className="px-2 py-1 bg-white/5 text-xs rounded text-white/70">
                      {tech}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-white/40" itemProp="datePublished">{project.year}</span>
                ...
                {project.results && (
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <p className="text-sm text-white/70">
                      <span className="font-medium text-cyan-400">Results:</span> {project.results}
                    </p>
                  </div>
                )}
                
                <meta itemProp="creator" content={agencyInfo.name} />
              </div>
            </article>
          ))}
        </div>
            ...
            "mainEntity": {
              ...
              "itemListElement": projects.map((project, index) => ({
                ...
                "item": {
                  ...
                  "url": project.liveUrl || `${agencyInfo.website}/projects#${project.id}`,
                  "datePublished": project.year,
                  "creator": {
                    "@type": "Organization",
                    "name": agencyInfo.name
                  },
                  "image": project.image ? `${agencyInfo.website}${project.image}` : `${agencyInfo.website}/clispy-solutions-logo.png`
                }
              }))
            }
          })
